...
//				companyReview = handleCompanyReviewExtendsResultSet(rset);
	...
	private CompanyReview handleCompanyReviewResultSet(ResultSet rset) throws SQLException {
		...
		return new CompanyReview(no, uid, companyNo, categoryNumber, content, stars, 
				workLifeBalance, levelUp, workIntensity, potential, salarySatisfaction, regDate);
	}
	...
	public List<CompanyReview> findAll(Connection conn, Map<String, Object> param) {
		...
		try {
			...
			while(rset.next()) {
				CompanyReview companyReview = handleCompanyReviewResultSet(rset);
